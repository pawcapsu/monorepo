...
export class ChaptersService {
  ...
  public async fetchDescription(
    ...
    return (
      (await this.textService.fetchText(chapter?.description)) || {
        _id: null,
        version: null,
        nodes: [],
      }
  ...
  // fetchPositions
  public async fetchPositions(bookId: ObjectId): Promise<ObjectId[]> {
    const book = await this.bookService.fetchBook(bookId);
    const chapters = await this.fetchBookChapters(bookId);

    return [
      ...book.chaptersPositions,
      ...chapters
        .filter((x) => !book.chaptersPositions?.includes(x._id))
        .map((x) => x._id),
    ];
  }

  // _positionChapters
  public _positionChapters(chapters: BookChapter[], positions: ObjectId[]) {
  ...
  // calculateBookSize
  public async calculateBookSize(bookId: ObjectId): Promise<Number> {
    const chapters = await this.chapterModel.find({ bookId }).exec();
