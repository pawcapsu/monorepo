...
export class UniversalTextService {
  ...
  // createText
  async createText(nodes: Array<UNodeEntity>): Promise<UniversalText> {
  ...
  // deleteNode
  async deleteNode(user: Profile, id: ObjectId, nodeId: number) {
    const text = await this.fetchText(id);
  ...
  async moveNode(
    ...
    if (text !== null) {
      ...
      if (this.permissionsService._checkTextWritePermissions(user._id, text)) {
        // +todo
        const fromNode = Object.assign(
          {},
          nodes.find((x) => x.index === fromNodeId)
        );
        const toNode = Object.assign(
          {},
          nodes.find((x) => x.index === toNodeId)
        );
        ...
        if (fromNode && toNode) {
          ...
          throw new HttpException(
            "Invalid fromNodeId or toNodeId",
            HttpStatus.BAD_REQUEST
          );
        }
      } else {
  ...
  // fetchText
  async fetchText(id: ObjectId): Promise<UniversalText | null> {
  ...
  public _prepareText(text: UniversalText): UniversalText | undefined {
    // preparing nodes
  ...
  // _indexNodes
  public _indexNodes(nodes: Array<UNodeEntity>): Array<UNodeEntity> {
    let index = 0;
