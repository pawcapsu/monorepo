...
import {
  BookTagInformationInput,
  TagFilterOptionsInput,
} from "@pawcapsu/types/dto/interactions";
...
export class BookTagResolver {
  ...
  public async getBookTag(@Args("id") id: string) {
  ...
  public async resolveCreator(
    ...
    if (tag.creator) {
      return await this.profilesService.findProfile(
        tag.creator as mongoose.Schema.Types.ObjectId
      );
    } else {
      return undefined;
    }
  ...
  public async resolveTagDescription(
    ...
    if (tag.description) {
      return await this.textService.fetchText(
        tag.description as mongoose.Schema.Types.ObjectId
      );
    } else {
      return undefined;
    }
  ...
  public async resolveTagBook(@Parent() tag: BookTag) {
    return await this.bookService.fetchBook(
      tag.book as mongoose.Schema.Types.ObjectId
    );
  }
}
