...
import { BookSearchOptionsInput } from "@pawcapsu/types/dto/content/Book/SearchOptions.dto";
import { PaginatedBooks } from "@pawcapsu/types/models/content/Book/PaginatedBooks.model";
import { TagService } from "@pawcapsu/modules/interactions/tag/services";
import { TagFilterOptionsInput } from "@pawcapsu/types/dto/interactions";
...
export class BooksResolver {
  ...
  @Query((returns) => Book)
  async book(@Args("bookId", { description: "Book _id" }) id: string) {
  ...
  async resolveBookRatings(
    ...
    @Args("limit", {
      nullable: true,
      description: "Number of ratings we need to get",
    })
    limit?: number,
    @Args("direction", {
      nullable: true,
      description: "Is it a Like or a Dislike?",
      type: () => EUserRatingDirection,
    })
    direction?: EUserRatingDirection
  ) {
    return this.ratingService.fetchRatings(book._id, EUserRatingType.BOOK, {
      limit,
      direction,
    });
  }
  ...
  async resolveBookChapters(
    ...
    @Args("limit", {
      nullable: true,
      description: "Number of chapters we need to get",
    })
    limit: number
  ) {
    const positions = await this.chaptersService.fetchPositions(book._id);
    const chapters = this.chaptersService._positionChapters(
      await this.chaptersService.fetchBookChapters(book._id, { limit }),
      positions
    );
    return chapters;
  }
