...
export class BookRatingResolver {
  ...
  async getBookRating(
    @Args("ratingId", { description: "UserRating _id property" })
    ratingId: string
  ) {
  ...
  async resolveUserField(@Parent() rating: BookRating) {
    return await this.profilesService.findProfile(
      rating.user as mongoose.Schema.Types.ObjectId
    );
  }

  // resolveEntityField
  @ResolveField("book", (retuns) => Book)
  async resolveEntityField(@Parent() rating: BookRating) {
